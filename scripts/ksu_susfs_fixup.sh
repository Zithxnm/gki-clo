...
if [ "$MANAGER" = "resukisu" ]; then
    ...
    if [ -f "./fs/susfs.c" ]; then
        sed -i 's/static void susfs_run_sus_path_loop(void)/void susfs_run_sus_path_loop(void)/g' "./fs/susfs.c"
...
fix_ksu_next_bridge() {
    ...
    if grep -q "ksu_handle_setresuid(old_uid, current_uid().val)" "$BRIDGE_C" 2>/dev/null; then
        ...
        ksu_handle_setresuid(ruid, euid, suid);\
            ...
            if ! grep -q "extern int ksu_handle_setresuid" "$BRIDGE_C" 2>/dev/null; then
                sed -i '1a extern int ksu_handle_setresuid(uid_t ruid, uid_t euid, uid_t suid);' "$BRIDGE_C"
            fi
            echo "[SUSFS-Fixup] syscall_event_bridge.c: Fixed setresuid 3-arg and added declaration"
        else
...
void ksu_handle_vfs_fstat(int fd, loff_t *kstat_size_ptr) { }' "$KSUD_INT_C"
        echo "[SUSFS-Fixup] ksud_integration.c: Restored ksu_sys_fstat and added ksu_handle_vfs_fstat stub"
    fi
...
fix_dirty_sepolicy() {
    ...
    for candidate in \
        "./KernelSU" \
        "./.root_modules/KernelSU-Next" \
        "./.root_modules/sukisu-ultra" \
        "./.root_modules/ReSukiSU"; do
    ...
    # Step 2: Make required symbols non-static (SUSFS selinuxfs.c expects them to be exported)
    ...
    sed -i 's/^static DEFINE_STATIC_KEY_FALSE(fake_status_initialize_key);/DEFINE_STATIC_KEY_FALSE(fake_status_initialize_key);/' "$SELINUX_HIDE_C"
    sed -i 's/^static struct page \*fake_status = NULL;/struct page \*fake_status = NULL;/' "$SELINUX_HIDE_C"
    sed -i 's/^static void initialize_fake_status()/void initialize_fake_status()/' "$SELINUX_HIDE_C"
...
fix_backup_sepolicy_leak() {
    ...
    [ ! -f "$RULES_C" ] && RULES_C="KernelSU/kernel/selinux/rules.c"
...
fix_dirty_sepolicy_seqno() {
    ...
    [ ! -f "$RULES_C" ] && RULES_C="KernelSU/kernel/selinux/rules.c"
    [ ! -f "$HIDE_C" ] && HIDE_C="KernelSU/kernel/feature/selinux_hide.c"
...
# --------------------------------------------------------------------------
# Fix missing _with_policy symbols in Android 6.6
# SUSFS patches inject these but some branches omit them
# --------------------------------------------------------------------------
if [ -f "security/selinux/selinuxfs.c" ] && grep -q "security_context_to_sid_with_policy" "security/selinux/selinuxfs.c" 2>/dev/null; then
    sed -i 's/security_context_to_sid_with_policy(backup_sepolicy, /security_context_to_sid(/g' security/selinux/selinuxfs.c security/selinux/hooks.c
    sed -i 's/, SECSID_NULL//g' security/selinux/selinuxfs.c security/selinux/hooks.c
    sed -i 's/security_sid_to_context_with_policy(backup_sepolicy, /security_sid_to_context(/g' security/selinux/selinuxfs.c security/selinux/hooks.c
    sed -i 's/security_compute_av_user_with_policy(backup_sepolicy, /security_compute_av_user(/g' security/selinux/selinuxfs.c security/selinux/hooks.c
    echo "[SUSFS-Fixup] selinuxfs.c & hooks.c: Dropped _with_policy suffix and backup_sepolicy arg"
fi
